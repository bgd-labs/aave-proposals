...
/**
 * @dev Test for AaveV3_Optimism_StataTokenOperationalUpdate_20230815
 * command: make test-contract filter=AaveV3_Optimism_StataTokenOperationalUpdate_20230815
 */
contract AaveV3_Optimism_StataTokenOperationalUpdate_20230815_Test is ProtocolV3TestBase {
  AaveV3_Optimism_StataTokenOperationalUpdate_20230815 internal proposal;
  ...
  function testProposalExecution() public {
    ...
    for (uint256 i; i < tokens.length; i++) {
      ...
      TransparentUpgradeableProxy(payable(tokens[i])).changeAdmin(AaveMisc.PROXY_ADMIN_OPTIMISM);
    }
    vm.startPrank(AaveMisc.PROXY_ADMIN_OPTIMISM);
    for (uint256 i; i < tokens.length; i++) {
      assertEq(
