...
import {IVlToken} from './interfaces/IVlToken.sol';
...
abstract contract VlTokenManager is Common {
    ...
    function lockVLAURA(uint256 amount) external onlyOwnerOrGuardian {
        ...
        (uint256 lockedBalance, uint256 lockHorizon,,) = IVlToken(VL_AURA).lockedBalances(address(this));

        emit LockVLAURA(lockedBalance, lockHorizon);
    }
