...
contract AaveV3ArbMAICapsUpdates_20230724_Test is ProtocolV3TestBase {
    ...
    function testPayload() public {
        ...
        ReserveConfig[] memory allConfigsBefore =
            createConfigurationSnapshot("preAaveV3ArbMAICapsUpdates_20230724Change", AaveV3Arbitrum.POOL);

        // 2. execute payload
        GovHelpers.executePayload(vm, address(arbitrumPayload), AaveGovernanceV2.ARBITRUM_BRIDGE_EXECUTOR);

        // 3. create snapshot after payload execution
        ReserveConfig[] memory allConfigsAfter =
            createConfigurationSnapshot("postAaveV3ArbMAICapsUpdates_20230724Change", AaveV3Arbitrum.POOL);

        // 4. Verify payload:
        ReserveConfig memory MAI_UNDERLYING_CONFIG =
        ...
        // 5. compare snapshots
        diffReports("preAaveV3ArbMAICapsUpdates_20230724Change", "postAaveV3ArbMAICapsUpdates_20230724Change");

        _noReservesConfigsChangesApartFrom(allConfigsBefore, allConfigsAfter, AaveV3ArbitrumAssets.MAI_UNDERLYING);
    }
}
...
contract AaveV3PolMAICapsUpdates_20230724_Test is ProtocolV3TestBase {
    ...
    function testPayload() public {
        ...
        // 1. create snapshot before payload execution
        ReserveConfig[] memory allConfigsBefore =
            createConfigurationSnapshot("preAaveV3PolMAICapsUpdates_20230724Change", AaveV3Polygon.POOL);

        // 2. execute payload
        GovHelpers.executePayload(vm, address(polygonPayload), AaveGovernanceV2.POLYGON_BRIDGE_EXECUTOR);

        // 3. create snapshot after payload execution
        ReserveConfig[] memory allConfigsAfter =
            createConfigurationSnapshot("postAaveV3PolMAICapsUpdates_20230724Change", AaveV3Polygon.POOL);

        // 4. Verify payload:
        ReserveConfig memory MAI_UNDERLYING_CONFIG =
            _findReserveConfig(allConfigsBefore, AaveV3PolygonAssets.miMATIC_UNDERLYING);
        ...
        // 5. compare snapshots
        diffReports("preAaveV3PolMAICapsUpdates_20230724Change", "postAaveV3PolMAICapsUpdates_20230724Change");

        _noReservesConfigsChangesApartFrom(allConfigsBefore, allConfigsAfter, AaveV3PolygonAssets.miMATIC_UNDERLYING);
    }
}
...
contract AaveV3OptMAICapsUpdates_20230724_Test is ProtocolV3TestBase {
    ...
    function testPayload() public {
        ...
        // 1. create snapshot before payload execution
        ReserveConfig[] memory allConfigsBefore =
            createConfigurationSnapshot("preAaveV3OptMAICapsUpdates_20230724Change", AaveV3Optimism.POOL);

        // 2. execute payload
        GovHelpers.executePayload(vm, address(optimismPayload), AaveGovernanceV2.OPTIMISM_BRIDGE_EXECUTOR);

        // 3. create snapshot after payload execution
        ReserveConfig[] memory allConfigsAfter =
            createConfigurationSnapshot("postAaveV3OptMAICapsUpdates_20230724Change", AaveV3Optimism.POOL);
        ...
        // 5. compare snapshots
        diffReports("preAaveV3OptMAICapsUpdates_20230724Change", "postAaveV3OptMAICapsUpdates_20230724Change");

        _noReservesConfigsChangesApartFrom(allConfigsBefore, allConfigsAfter, AaveV3OptimismAssets.MAI_UNDERLYING);
    }
}
...
contract AaveV3AvaxMAICapsUpdates_20230724_Test is ProtocolV3TestBase {
    ...
    function testPayload() public {
        ...
        // 1. create snapshot before payload execution
        ReserveConfig[] memory allConfigsBefore =
            createConfigurationSnapshot("preAaveV3AvaxMAICapsUpdates_20230724Change", AaveV3Avalanche.POOL);

        // 2. execute payload
        // GovHelpers.executePayload(vm, address(avalanchePayload), AaveGovernanceV2.AVALANCHE_BRIDGE_EXECUTOR);

        // vm.startPrank(GUARDIAN_AVALANCHE);

        // IACLManager aclManager = AaveV3Avalanche.ACL_MANAGER;
        // aclManager.addAssetListingAdmin(address(avalanchePayload));
        // aclManager.addRiskAdmin(address(avalanchePayload));
        // avalanchePayload.execute();

        // vm.stopPrank();

        // 3. create snapshot after payload execution
        ReserveConfig[] memory allConfigsAfter =
            createConfigurationSnapshot("postAaveV3AvaxMAICapsUpdates_20230724Change", AaveV3Avalanche.POOL);

        // 4. Verify payload:
        ReserveConfig memory MAI_UNDERLYING_CONFIG =
            _findReserveConfig(allConfigsBefore, AaveV3AvalancheAssets.MAI_UNDERLYING);
        ...
        // 5. compare snapshots
        diffReports("preAaveV3AvaxMAICapsUpdates_20230724Change", "postAaveV3AvaxMAICapsUpdates_20230724Change");

        _noReservesConfigsChangesApartFrom(allConfigsBefore, allConfigsAfter, AaveV3AvalancheAssets.MAI_UNDERLYING);
    }
}
